...
describe('POST /api/upload/image - Upload Gambar', () => {
  beforeEach(() => jest.clearAllMocks());

  it('mengembalikan URL saat upload berhasil', async () => {
  ...
  it('schema zod menolak nilai non-File (field file kosong)', () => {
    // Test schema validation langsung — membuktikan schema menolak bukan-File
    const z = require('zod');
    const schema = z.object({
      file: z
        .any()
        .refine((v: unknown): v is File => v instanceof File, {
  ...
  it('schema zod menolak file yang melebihi batas 2MB', () => {
    const z = require('zod');
    const schema = z.object({
      file: z
        .any()
        ...
        .refine((f: File) => f.size <= 2 * 1024 * 1024, {
  ...
  it('mengembalikan 500 dengan pesan error saat ImageKit gagal', async () => {
  ...
  it('menggunakan folder default /crafter ketika IMAGEKIT_FOLDER tidak diatur di env', async () => {
