...
export default function PersonaItems() {
  ...
  const [myPersonasOnly, setMyPersonasOnly] = useState(initialMine);
  ...
  async function fetchPersonas() {
    ...
    try {
      ...
      if (myPersonasOnly) qs.set('mine', '1');
  ...
  // fetch on first mount and whenever dependencies change
  useEffect(() => {
    fetchPersonas();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [page, debouncedSearch, domain, order, myPersonasOnly]);

  // reset page when filters change (not when page itself changes)
  useEffect(() => {
    setPage(1);
  }, [debouncedSearch, domain, order, myPersonasOnly]);
  ...
  }, [pathname, router, page, debouncedSearch, domain, order, myPersonasOnly]);
