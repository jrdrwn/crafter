...
export async function createEmbeddingsClient(provider?: 'gemini' | 'openai') {
  const prefer = provider ?? (process.env.EMBEDDING_PROVIDER as any) ?? (process.env.OPENAI_API_KEY ? 'openai' : 'gemini');
  if (prefer === 'openai') {
    const mod = await import('@langchain/openai');
  ...
  return new GoogleGenerativeAIEmbeddings({ apiKey: process.env.GEMINI_API_KEY });
}
